...
exports.getTasks = async (req, res) => {
    try {
        const tasks = await Task.find({ userId: req.user._id }).sort({ scheduledDate: 1, deadline: 1 });
...
exports.getWeeklyPlan = async (req, res) => {
    try {
        const plan = await scheduler.generateWeeklyPlan(req.user._id);
        res.json(plan);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
exports.getProductivity = async (req, res) => {
    try {
        const score = await scheduler.calculateProductivity(req.user._id);
...
exports.autoReschedule = async (req, res) => {
    try {
        const updatedCount = await scheduler.rescheduleMissedTasks(req.user._id);
