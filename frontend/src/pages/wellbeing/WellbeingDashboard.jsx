...
export function WellbeingDashboard() {
  ...
  useEffect(() => {
    const fetchOverview = async () => {
      try {
        ...
        const res = await fetch("/api/integration/student-overview", {
        ...
        const histRes = await fetch("/api/moods", {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (histRes.ok) {
          const hData = await histRes.json();

          const moodScoreMap = { happy: 5, calm: 4, neutral: 3, stressed: 2, sad: 1, overwhelmed: 1 };

          const formatted = hData.reverse().map(entry => {
            const d = new Date(entry.date);
            return {
              day: d.toLocaleDateString([], { weekday: 'short' }),
              score: moodScoreMap[entry.mood] || 3
            };
          });

          // Ensure we have at least some data points for a smooth line chart
          if (formatted.length === 0) {
            setMoodHistory([{ day: "Today", score: 3 }]);
          } else if (formatted.length === 1) {
            setMoodHistory([{ day: "Prev", score: formatted[0].score }, formatted[0]]);
          } else {
  ...
  const capitalize = (str) => {
    if (!str || str === "Not logged") return str;
  ...
  return <div className="space-y-6 animate-fade-in">{
    ...
    Good morning, Biseka ! <span className="text-2xl">👋</span></h1><p className="text-slate-500 mt-1">
      Here's your wellbeing overview for today.
    </p></div><div className="flex items-center gap-3"><Button
      variant="secondary"
      onClick={() => navigate("/wellbeing/support")}
      className="gap-2"
    ><ShieldAlert className="w-4 h-4 text-rose-500" />
      Anonymous Support
    </Button><Button
      variant="primary"
      onClick={() => navigate("/wellbeing/mood")}
      className="gap-2"
    ...
    ><CardContent className="p-5"><div className="flex justify-between items-start mb-4"><div className="p-2 bg-indigo-100 rounded-xl text-indigo-600"><Smile className="w-5 h-5" /></div><Badge variant={stats.latestMood !== "Not logged" ? "success" : "neutral"}>
      {stats.latestMood !== "Not logged" ? "Logged Today" : "Needs Update"}
    </Badge></div><h3 className="text-sm font-medium text-slate-500">Today's Mood</h3><div className="mt-1 flex items-baseline gap-2"><span className="text-2xl font-bold text-slate-800">
      {capitalize(stats.latestMood)}
    </span><span className="text-xl">{getMoodEmoji(stats.latestMood)}</span></div></CardContent></Card><Card
      hoverable
      className="bg-gradient-to-br from-rose-50 to-white border-rose-100/50"
    ><CardContent className="p-5"><div className="flex justify-between items-start mb-4"><div className="p-2 bg-rose-100 rounded-xl text-rose-600"><Activity className="w-5 h-5" /></div><Badge variant={stats.latestStressLevel > 3 ? "warning" : "success"}>
      {stats.latestStressLevel > 3 ? "Elevated" : "Manageable"}
    </Badge></div><h3 className="text-sm font-medium text-slate-500">
            Current Stress
          </h3><div className="mt-1 flex items-baseline gap-2"><span className="text-2xl font-bold text-slate-800">
            {stats.latestStressLevel !== "Not logged" ? `Level ${stats.latestStressLevel}` : "No Data"}
          </span>
            {stats.latestStressLevel !== "Not logged" && <span className="text-sm text-slate-500">/ 5</span>}
          </div></CardContent></Card><Card
            hoverable
            className="bg-gradient-to-br from-teal-50 to-white border-teal-100/50"
            onClick={() => navigate("/wellbeing/risk")}
          ><CardContent className="p-5"><div className="flex justify-between items-start mb-4"><div className="p-2 bg-teal-100 rounded-xl text-teal-600"><HeartPulse className="w-5 h-5" /></div><Badge variant={stats.burnoutRisk === "High" ? "danger" : stats.burnoutRisk === "Moderate" ? "warning" : "success"}>
            {stats.burnoutRisk} Risk
          </Badge></div><h3 className="text-sm font-medium text-slate-500">
            AI Burnout Status
          </h3><div className="mt-1 flex items-baseline gap-2"><span className="text-2xl font-bold text-slate-800">{stats.burnoutRisk}</span></div></CardContent></Card><Card
            hoverable
            className="bg-gradient-to-br from-purple-50 to-white border-purple-100/50"
            onClick={() => navigate("/wellbeing/counseling")}
          ><CardContent className="p-5"><div className="flex justify-between items-start mb-4"><div className="p-2 bg-purple-100 rounded-xl text-purple-600"><CalendarIcon className="w-5 h-5" /></div><Badge variant="neutral">Upcoming</Badge></div><h3 className="text-sm font-medium text-slate-500">Next Session</h3><div className="mt-1 flex items-baseline gap-2">
            <span className="text-lg font-bold text-slate-800 truncate">
              {stats.nextSessionCounselor || "No session"}
            </span>
            {stats.nextSessionDate && (
              <span className="text-sm text-slate-500">{stats.nextSessionDate}</span>
      ...
      variant="ghost"
      size="sm"
      onClick={() => navigate("/wellbeing/analytics")}
    ...
    >
      ...
      ><CartesianGrid
        ...
        >
            ...
            Need to talk?
              ...
              variant="primary"
              className="flex-1"
              onClick={() => navigate("/wellbeing/chat")}
            >
      ...
      ><div className="flex items-center gap-3"><div className="p-2 bg-indigo-50 rounded-lg text-indigo-600 group-hover:bg-indigo-100 transition-colors"><Smile className="w-4 h-4" /></div><span className="text-sm font-medium text-slate-700">
        ...
        onClick={() => navigate("/wellbeing/stress")}
        className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all group"
