      ...
      <div className="max-w-md mx-auto mt-12 animate-fade-in relative z-10">
        <Card className="text-center py-12 border-emerald-100 bg-emerald-50/30">
          <CardContent className="flex flex-col items-center">
            ...
            <Button variant="outline" onClick={() => navigate("/wellbeing")}>Return to Dashboard</Button>
          </CardContent>
        </Card>
      </div>
    );
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-2">
              <Textarea
                ...
                className="bg-white"
              />
              <p className="text-xs text-slate-400 text-right">Your notes are private and encrypted.</p>
            </div>
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-3">
            {history.map((entry) => {
              ...
              const dateStr = new Date(entry.date).toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' });
              return (
                <div key={entry._id} className="flex items-center gap-4 p-4 rounded-xl bg-white/90 backdrop-blur-sm border border-slate-100 shadow-sm">
                  ...
                  <div>
                    ...
                    {entry.note && <p className="text-sm text-slate-500 mt-0.5 line-clamp-1">{entry.note}</p>}
                  </div>
                </div>
              );
            })}
