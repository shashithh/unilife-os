...
import { ArrowLeft, Calendar as CalendarIcon, Clock, Video, MapPin, CheckCircle2 } from "lucide-react";
...
export function CounselorSlotSelection() {
  ...
  const handleReasonChange = (e) => {
    ...
    if (value.length > 50) {
      setReasonError("Reason must be 50 characters or less");
    } else {
      setReasonError("");
    }
  };
              ...
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-200">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-indigo-100 flex items-center justify-center text-indigo-500 font-bold text-xl">
                  {counselor.profileImage ? (
                    <img src={counselor.profileImage} alt="" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(counselor.counselingModes || ["Online", "In-person"]).includes("Online") && (
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-medium transition-all ${sessionMode === "Online"
                  ...
                  {(counselor.counselingModes || ["Online", "In-person"]).includes("In-person") && (
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-medium transition-all ${sessionMode === "In-person"
                          ? "bg-indigo-50 border-indigo-500 text-indigo-700 ring-1 ring-indigo-500"
                          : "bg-white border-slate-200 text-slate-600"
                        }`}
                    >
              ...
              <div>
                ...
                <div className="flex justify-between mt-1">
                  {reasonError ? (
                    ...
                    <p className="text-xs text-slate-400">Maximum 50 characters</p>
                  )}
                  <p className="text-xs text-slate-400">{reason.length}/50</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6 flex-1 flex flex-col">
              {availableSlots.length === 0 ? (
                <div className="text-center py-12 text-slate-500">
                  No available slots. Please try another counselor.
                </div>
              ) : (
                <>
                  <div className="flex gap-2 overflow-x-auto pb-4 mb-4 border-b border-slate-100">
                    {availableSlots.map((day) => (
                      <button
                        ...
                        className={`px-5 py-3 rounded-xl whitespace-nowrap transition-all ${selectedDate === day.date
                  ...
                  <div className="mb-8">
                    <h4 className="text-sm font-medium text-slate-700 mb-3 flex items-center gap-2">
                      <Clock className="w-4 h-4 text-slate-400" /> Available times
                    </h4>
                      ...
                      {availableSlots
                        ...
                        ?.slots.map((slot) => (
                          <button
                            ...
                            className={`py-3 rounded-xl border text-sm font-medium transition-all ${selectedSlot === slot
                                ? "bg-indigo-50 border-indigo-500 text-indigo-700 ring-2 ring-indigo-500/20 shadow-sm"
