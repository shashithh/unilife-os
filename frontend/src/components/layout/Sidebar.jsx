...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 h-screen bg-white/80 backdrop-blur-xl border-r border-gray-200 flex flex-col fixed left-0 top-0 z-20">
      ...
      <div className="p-4 m-4 rounded-xl bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-100 flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-blue-200 border-2 border-white shadow-sm flex items-center justify-center overflow-hidden">
            <User className="text-blue-500 w-5 h-5" />

          </div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-bold text-gray-800 truncate">{JSON.parse(localStorage.getItem("user") || "{}").fullName || "Student"}</p>
            <p className="text-xs text-gray-500 truncate">{JSON.parse(localStorage.getItem("user") || "{}").role || "Student"}</p>
          </div>
        </div>
        <button 
          onClick={() => {
          ...
          className="w-full flex items-center justify-center gap-2 py-2 text-sm text-red-600 font-medium bg-white/60 hover:bg-red-50 rounded-lg transition-colors border border-red-100"
        >
          <LogOut size={16} />
          <span>Logout</span>
        </button>
