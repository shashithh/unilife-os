...
export const Textarea = forwardRef(
  ({ label, error, helperText, className = "", ...props }, ref) => {
    return <div className={`flex flex-col gap-1.5 ${className}`}>{label && <label className="text-sm font-medium text-slate-700">{label}</label>}<textarea
      ref={ref}
      className={`
            px-4 py-3 rounded-xl border bg-white/50 backdrop-blur-sm transition-all duration-200 resize-y min-h-[100px]
